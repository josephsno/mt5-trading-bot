...
class RSIFlexibleStrategy_MACDReversal:
    ...
    def _get_lot_size(self, symbol: str, capped: int = 3) -> float:
        capped_streak = min(
            self.winning_streak, capped
        )  # cap at specified number of wins

        if self.backtest_mode:
            # Reset if loss OR streak has hit the cap
            if not self.last_trade_won or self.winning_streak > capped:
                self.current_lot = self.starting_lot
            elif self.last_trade_won:
                self.current_lot = self.starting_lot + 0.01 * (capped_streak - 1)
            else:
                self.current_lot = self.starting_lot
            return round(self.current_lot, 2)
            ...
            wins = self._get_consecutive_wins(symbol)
            # Reset if loss OR streak exceeded cap
            if wins == 0 or wins > capped:
                self.current_lot = self.starting_lot
            else:
                wins = min(wins, capped)
                self.current_lot = self.starting_lot + 0.01 * (wins - 1)
            print(
                f"{symbol} consecutive wins counted: {wins} (capped at {capped}) | lot set to {self.current_lot}"
            )
            return round(self.current_lot, 2)
...
class RSIFlexibleStrategy_MACDReversal_Trial2:
    """
    Flexible RSI Strategy + MACD Reversal Filter — Trial 2
    - RSI trend with moderate validations
    - EMA slope optional confirmation
    - Volume check optional
    - Dynamic lot sizing using winning streak (capped)
    - MACD CROSSOVER filter (stricter than Trial 1):
        BUY  → current histogram must be GREEN (> 0) AND prev must be RED (< 0)
               i.e. histogram just crossed zero upward this bar
        SELL → current histogram must be RED   (< 0) AND prev must be GREEN (> 0)
               i.e. histogram just crossed zero downward this bar

    This prevents entries mid-red-streak (Trial 1 bug) by requiring
    an actual zero-line crossover as confirmation.
    """
    ...
    def generate_signal(self, price_data: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        # ---------- MACD CROSSOVER FILTER (Trial 2) ----------
        # Requires an actual zero-line crossover on the current bar:
        # BUY  → curr histogram GREEN (> 0) AND prev histogram RED (< 0)
        # SELL → curr histogram RED   (< 0) AND prev histogram GREEN (> 0)
        # This blocks entries mid-streak (e.g. all-red sequence) which Trial 1 allowed
        curr_macd_hist = macd_hist.iloc[-1]
        prev_macd_hist = macd_hist.iloc[-2]

        if trend == "buy":
            if not (curr_macd_hist > 0 and prev_macd_hist < 0):
                return self._empty_signal(
                    f"❌ BUY blocked | no MACD crossover | "
                    f"curr={curr_macd_hist:.5f} ({'🟢' if curr_macd_hist > 0 else '🔴'}) "
                    f"prev={prev_macd_hist:.5f} ({'🟢' if prev_macd_hist > 0 else '🔴'}) "
                    f"— need curr🟢 + prev🔴"
                )
        ...
        if trend == "sell":
            if not (curr_macd_hist < 0 and prev_macd_hist > 0):
                return self._empty_signal(
                    f"❌ SELL blocked | no MACD crossover | "
                    f"curr={curr_macd_hist:.5f} ({'🟢' if curr_macd_hist > 0 else '🔴'}) "
                    f"prev={prev_macd_hist:.5f} ({'🟢' if prev_macd_hist > 0 else '🔴'}) "
                    f"— need curr🔴 + prev🟢"
                )
        ...
        return {
            ...
            "reason": (
                ...
                f"slope={ema_slope:.6f}, "
                f"RSI={rsi.iloc[-1]:.2f}, "
                f"MACD crossover: curr={curr_macd_hist:.5f} ({'🟢' if curr_macd_hist > 0 else '🔴'}) "
                f"prev={prev_macd_hist:.5f} ({'🔴' if prev_macd_hist < 0 else '🟢'}), "
                f"volume_ratio={volume_ratio if volume_ratio else 'OK'}"
            ),
        }
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            "name": "RSI Flexible Strategy + MACD Reversal — Trial 2",
            "sl_pips": self.sl_pips,
            "tp_pips": self.tp_pips,
            "rsi_period": self.rsi_period,
            "rsi_buy_level": self.rsi_buy_level,
            ...
            "macd_signal": self.macd_signal_period,
            # Trial 2 specific
            "macd_filter": "crossover (curr🟢+prev🔴 for BUY | curr🔴+prev🟢 for SELL)",
        }

    def __repr__(self) -> str:
        return (
            f"RSIFlexibleStrategy_MACDReversal_Trial2("
            f"RSI{self.rsi_period}, EMA{self.ema_trend}, "
            f"MACD({self.macd_fast},{self.macd_slow},{self.macd_signal_period}), "
            f"SL={self.sl_pips}, TP={self.tp_pips})"
        )
...
class RSIFlexibleStrategy_MACDReversal_Trial3:
    ...
    - Volume check optional
    - Dynamic lot sizing using winning streak (capped)
    - MACD CROSSOVER + WAVE STRENGTH filter (stricter than Trial 2):
        BUY  → current histogram GREEN (> 0) AND prev was RED (< 0)  [crossover]
               AND the previous RED wave lasted >= macd_min_wave_length bars
               AND the previous RED wave peaked >= macd_min_wave_peak depth
        SELL → current histogram RED (< 0) AND prev was GREEN (> 0)  [crossover]
               AND the previous GREEN wave lasted >= macd_min_wave_length bars
               AND the previous GREEN wave peaked >= macd_min_wave_peak height

    Prevents entries on blip crossovers (1-2 bar flickers).
    Only trades when the previous wave was a real mountain — sustained and deep.
    """
    ...
    def __init__(
        ...
        # MACD wave strength (Trial 3)
        # min bars the previous wave must have lasted
        self.macd_min_wave_length: int = 3
        # min absolute peak the previous wave must have reached
        # tune per symbol: ~0.0003 for USD pairs, ~0.05 for JPY, ~0.5 for XAU
        self.macd_min_wave_peak: float = 0.0003
    ...
    def _get_prev_wave_stats(
        ...
        For BUY  → count consecutive RED bars  (< 0) before the crossover
        For SELL → count consecutive GREEN bars (> 0) before the crossover

        Returns:
            wave_length : number of bars in that wave
            wave_peak   : absolute max value (depth/height) reached in that wave
        """
        wave_length = 0
        wave_peak = 0.0
        ...
        for i in range(2, len(macd_hist)):
            ...
            if trend == "buy" and val < 0:
                wave_length += 1
                wave_peak = max(wave_peak, abs(val))
            elif trend == "sell" and val > 0:
                wave_length += 1
                wave_peak = max(wave_peak, abs(val))
            else:
                break  # wave ended — stop counting

        return wave_length, wave_peak
    ...
    def _get_lot_size(self, symbol: str, capped: int = 9) -> float:
        capped_streak = min(self.winning_streak, capped)
    ...
    def generate_signal(self, price_data: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        # ---------- MACD CROSSOVER + WAVE STRENGTH FILTER (Trial 3) ----------
        curr_macd_hist = macd_hist.iloc[-1]
        prev_macd_hist = macd_hist.iloc[-2]

        # Step 1 — actual zero-line crossover this bar
        if trend == "buy":
            if not (curr_macd_hist > 0 and prev_macd_hist < 0):
                return self._empty_signal(
                    f"❌ BUY blocked | no crossover | "
                    f"curr={curr_macd_hist:.5f} ({'🟢' if curr_macd_hist > 0 else '🔴'}) "
                    f"prev={prev_macd_hist:.5f} ({'🟢' if prev_macd_hist > 0 else '🔴'})"
                )
        ...
        if trend == "sell":
            if not (curr_macd_hist < 0 and prev_macd_hist > 0):
                return self._empty_signal(
                    f"❌ SELL blocked | no crossover | "
                    f"curr={curr_macd_hist:.5f} ({'🟢' if curr_macd_hist > 0 else '🔴'}) "
                    f"prev={prev_macd_hist:.5f} ({'🟢' if prev_macd_hist > 0 else '🔴'})"
                )

        # Step 2 — previous wave must be a mountain: sustained + deep/tall
        wave_length, wave_peak = self._get_prev_wave_stats(macd_hist, trend)

        if wave_length < self.macd_min_wave_length:
            return self._empty_signal(
                f"❌ {'BUY' if trend == 'buy' else 'SELL'} blocked | "
                f"prev wave too short | length={wave_length} < {self.macd_min_wave_length} bars"
            )

        if wave_peak < self.macd_min_wave_peak:
            return self._empty_signal(
                f"❌ {'BUY' if trend == 'buy' else 'SELL'} blocked | "
                f"prev wave too weak | peak={wave_peak:.5f} < {self.macd_min_wave_peak}"
            )
        ...
        return {
            ...
            "reason": (
                ...
                f"slope={ema_slope:.6f}, "
                f"RSI={rsi.iloc[-1]:.2f}, "
                f"MACD crossover: curr={curr_macd_hist:.5f}🟢 prev={prev_macd_hist:.5f}🔴, "
                f"wave_length={wave_length} wave_peak={wave_peak:.5f}, "
                f"volume_ratio={volume_ratio if volume_ratio else 'OK'}"
            ),
        }
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            "name": "RSI Flexible Strategy + MACD Reversal — Trial 3",
            "sl_pips": self.sl_pips,
            "tp_pips": self.tp_pips,
            "rsi_period": self.rsi_period,
            "rsi_buy_level": self.rsi_buy_level,
            ...
            "macd_signal": self.macd_signal_period,
            # Trial 3 specific
            "macd_filter": "crossover + wave strength",
            "macd_min_wave_length": self.macd_min_wave_length,
            "macd_min_wave_peak": self.macd_min_wave_peak,
        }

    def __repr__(self) -> str:
        return (
            f"RSIFlexibleStrategy_MACDReversal_Trial3("
            f"RSI{self.rsi_period}, EMA{self.ema_trend}, "
            f"MACD({self.macd_fast},{self.macd_slow},{self.macd_signal_period}), "
            f"SL={self.sl_pips}, TP={self.tp_pips}, "
            f"wave≥{self.macd_min_wave_length}bars peak≥{self.macd_min_wave_peak})"
        )
